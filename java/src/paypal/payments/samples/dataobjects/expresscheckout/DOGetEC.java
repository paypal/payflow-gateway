	...
	public static void main(String args[]) {
		...
		if (Resp != null) {
			...
			if (TrxnResponse != null) {
				...
				// Express Checkout Transactions and Express Checkout Reference Transactions
				// with Purchase
				// begin with EC, while Express Checkout Reference Transactions without Purchase
				// begin with BA.
				...
				if (Trans.getResponse().getEcSetResponse().getToken() != null) {
					if (Trans.getResponse().getEcSetResponse().getToken().startsWith("EC")) {
						...
						System.out.println(
								"SHIPTOLASTNAME = " + Trans.getResponse().getEcGetResponse().getShipToLastName());
						System.out.println("SHIPTOSREET = " + Trans.getResponse().getEcGetResponse().getShipToStreet());
						System.out.println(
								"SHIPTOSTREET2 = " + Trans.getResponse().getEcGetResponse().getShipToStreet2());
						...
						System.out.println(
								"SHIPTOCOUNTRY = " + Trans.getResponse().getEcGetResponse().getShipToCountry());
						System.out.println("AVSADDR = " + Trans.getResponse().getTransactionResponse().getAvsAddr());
					}
					// ba_Flag is returned with Express Checkout Reference Transaction with
					// Purchase.
