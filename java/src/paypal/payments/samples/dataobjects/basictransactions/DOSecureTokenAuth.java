...
public class DOSecureTokenAuth {
	...
	public static void main(String args[]) {
		...
		// Since we are using the hosted payment pages, you will not be sending the
		// credit card data with the
		// Secure Token Request. You just send all other 'sensitive' data within this
		// request and when you
		// call the hosted payment pages, you'll only need to pass the SECURETOKEN;
		// which is generated and returned
		// and the SECURETOKENID that was created and used in the request.

		// Create a new Secure Token Authorization Transaction. Even though this example
		// is performing
		// an authorization, you can create a secure token using SaleTransaction too.
		// Only Authorization and Sale
		// type transactions are permitted.

		// Create a new Auth Transaction.
		AuthorizationTransaction trans = new AuthorizationTransaction(user, connection, inv, null,
				PayflowUtility.getRequestId());

		// Set the flag to create a Secure Token.
		trans.setCreateSecureToken("Y");
		// The Secure Token Id must be a unique id up to 36 characters. Using the
		// RequestID object to
