...
namespace PayPal.Payments.Samples.CS.DataObjects.BasicTransactions
{
	...
	public class DOSaleCompleteCS
	{
		...
		public static void Main(string[] Args)
		{
			...
			// Manager at https://manager.paypal.com or on the PayPal developers site at 
			// https://developer.paypal.com/docs/payflow/integration-guide/
			// 
			...
			// due to invalid account information or IP restriction on the account.  You can verify IPO restriction by logging 
			// into Manager.
            ...
            //CustInfo.CustCode = "Cust123";    // Customer Code/Customer Reference Id
            //Cust.CustIP = "255.255.255.255";  // Customer's ip address
			...
			// Card on File (COF) - Stored Credential
			...
			// See https://developer.paypal.com/docs/payflow/integration-guide/card-on-file/#supported-card-on-file-types for more information.
			//
			// Example: 
			// CITI (CIT Initial) - Signifies that the merchant is storing the cardholders credentials for the first time in anticipation of future 
			// stored credential transactions. Example: A cardholders sets up a customer profile for future purchases. 
			CC.CardonFile = "CITI";
			// You would send the TXID for all COF transactions; except "CITI".
			//CC.TxId = "X1Y2Z3100011713"; 
			...
			// be setup to process these types of transactions.  You will need to request your market segment be changed from e-commerce (default)
			// to retail.
			...
			while (trxCount <= 3 && !RespRecd)
			{
				...
				if (Resp != null)
				{
					...
					if (TrxnResponse != null)
					{
						...
						Console.WriteLine("Last 4-digits Credit Card Number (ACCT) = " + TrxnResponse.Acct);						if (TrxnResponse.CardType != null)
						{
					...
					else if (TrxnResponse.Result == 0)
					{
						...
						// In the message below it shows what failed, i.e. street, zip or cvv2.  To prevent fraud, it is suggested
